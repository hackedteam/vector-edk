...
UINT8 FfsEngine::saveImageFile(QModelIndex & index,int argc, char *argv[])
{
    ...
    for(i=argc-1;i>1;i--)
    {
        ...
        // path="C:\\uefi\\sorgenti\\edk_cod2\\Build\\OvmfX64\\RELEASE_VS2008x86\\FV\\Ffs\\eaea9aec-c9c1-46e2-9d52-432ad25a9b0bfsbg\\eaea9aec-c9c1-46e2-9d52-432ad25a9b0b.ffs";

         QFileInfo fileInfo = QFileInfo(argv[i]);
         if (!fileInfo.exists()) {
            msg((tr("Please select existing file")));
             return FALSE;
         }

         QFile inputFile;
         inputFile.setFileName(argv[i]);

         if (!inputFile.open(QFile::ReadOnly)) {
             //QMessageBox::critical(this, tr("Insertion failed"), tr("Can't open output file for reading"), QMessageBox::Ok);
             qDebug("Can't open output file for reading");
             return FALSE;
         }
         ...
         mode = CREATE_MODE_AFTER;
         result = insert(index, buffer, mode);
         if (result)
             //QMessageBox::critical(this, tr("Insertion failed"), tr("Error code: %1").arg(result), QMessageBox::Ok);
             qDebug("Error Insert Module %s",argv[i]);
         else
             //ui->actionSaveImageFile->setEnabled(true);
         {
            //qDebug("Insert Module %s",argv[i]);
         }
     ...
     path="C:\\uefi\\sorgenti\\edk_cod2\\Build\\OvmfX64\\RELEASE_VS2008x86\\FV\\Ffs\\f50258a9-2f4d-4da9-861e-bda84d07a44crkloader\\f50258a9-2f4d-4da9-861e-bda84d07a44c.ffs";
     ...
     if (result)
         ...
         //ui->actionSaveImageFile->setEnabled(true);
     {
        qDebug("OK Insert rkloader");
     }

     path="C:\\uefi\\sorgenti\\edk_cod2\\Build\\OvmfX64\\RELEASE_VS2008x86\\FV\\Ffs\\f50248a9-2f4d-4de9-86ae-bda84d07a41cNtfs\\f50248a9-2f4d-4de9-86ae-bda84d07a41c.ffs";
  ...
  ***/
   ...
   // UINT8 result = reconstructImageFile(reconstructed);
     result = reconstructImageFile(reconstructed);
    //showMessages();
    if (result) {
        //QMessageBox::critical(this, tr("Image reconstruction failed"), tr("Error code: %1").arg(result), QMessageBox::Ok);
        qDebug("Image reconstruction failed");
        return FALSE;
    }
...
void FfsEngine::SetFatIndex(QModelIndex & index)
{
    FatIndex = index;
}

void FfsEngine::SetPosIndex(QModelIndex & index)
{
    PosIndex = index;
}


QModelIndex FfsEngine::GetFatIndex()
{
        return FatIndex;
}

QModelIndex FfsEngine::GetPosIndex()
{
        return PosIndex;
}
...
/*
        ...
        msg(tr("index.child(i, 0)[[%1]][[%2]][[%3]][[%4]][[%5]]")
            .arg(model->nameString(index.child(0, 0)))
            .arg(model->textString(index.child(0, 0)))
            .arg(model->typeString(index.child(0, 0)))
            .arg(model->subtypeString(index.child(0, 0)))
            .arg(model->info(index.child(0, 0))));
*/


        //msg(tr("%1").arg(path.indexOf("3D17205B-4C49-47E2-8157-864CD3D80DBD")) );
        //msg( tr("%1").arg(path.indexOf("PE32+ image section")) );
/***
        if(path.indexOf("3D17205B-4C49-47E2-8157-864CD3D80DBD")!=-1 && path.indexOf("PE32+ image section") != -1)
        {
            //se entro qua è perche' sono dentro al MOD che contiene il nome del file di recovery
            qDebug("STOP000");

            int sizeMOD = model->body(index).size();
            char* buffer2 = model->body(index).data();


            //inizio pars per estrarre nome recovery

            const uint MaxValue = 4294967295;
            const uint MinValue = 0;

            //uint lOffset = Main.Find(ref buffer2, Encoding.ASCII.GetBytes("EL TORITO"), 0, 1);
            uint  lOffset = model->body(index).indexOf("EL TORITO");
            msg( tr("%1").arg(lOffset));

            if (lOffset != MaxValue)
            {

                int vvv = memcmp(buffer2 + lOffset - ((uint) 8L), "CD001",5);
                if(vvv==0)
                {
                   lOffset -= (uint) 9L;
                }
...
***/
/*
        //individuo il modulo recovery
        if(tr("%1").arg(model->nameString(index)) == "3D17205B-4C49-47E2-8157-864CD3D80DBD")
        {
            qDebug("STOP000");
        }
*/
    ...
    for (int i = 0; i < model->rowCount(index); i++) {
        ...
        /*
        msg(tr("[[%1]]").arg(model->textString(childIndex).isEmpty() ? model->nameString(childIndex) : model->textString(childIndex)));
*/

        //if(tr("%1").arg(model->nameString(childIndex)) == "C74233C1-96FD-4CB3-9453-55C9D77CE3C8")

        //mi posiziono dopo fat
        if(tr("%1").arg(model->nameString(childIndex)) == "961578FE-B6B7-44C3-AF35-6BC705CD2B1F")
        {
            //qDebug("STOP");
            SetFatIndex(childIndex);
            numPos=model->rowCount(index);
            //qDebug("no %x",numPos);
        }


    //    msg(tr("[[%1]][[%2]]").arg(model->nameString(childIndex)).arg(model->textString(childIndex)));


        if(i==numPos-1)
        {
            SetPosIndex(childIndex);
            numPos=-1;
        }
