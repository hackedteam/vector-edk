...
static int ntfs_ib_write(ntfs_index_context *icx, INDEX_BLOCK *ib)
{
	...
	ntfs_log_trace("vcn: %l\n", (long long)vcn);
	
	ret = ntfs_attr_mst_pwrite(icx->ia_na, ntfs_ib_vcn_to_pos(icx, vcn),
				   1, icx->block_size, ib);
	if (ret != 1) {
		ntfs_log_perror("Failed to write index block %l, inode %llu",
				       ...
				       ntfschar *name, u32 name_len)
{
	...
	if (ni->nr_extents == -1)
		ni = ni->base_ni;
	icx = (ntfs_index_context *) ntfs_calloc(sizeof(ntfs_index_context));
	if (icx)
	{
		icx->ni = ni;
		icx->name = name;
...
static INDEX_ENTRY *ntfs_ie_dup_novcn(INDEX_ENTRY *ie)
{
	...
	if (dup) {
		memcpy(dup, ie, size);
		dup->ie_flags &= (INDEX_ENTRY_FLAGS) ~INDEX_ENTRY_NODE;
...
static int ntfs_ia_check(ntfs_index_context *icx, INDEX_BLOCK *ib, VCN vcn)
{
	...
	if (!ntfs_is_indx_record(ib->magic)) {
		...
		ntfs_log_error("Corrupt index block signature: vcn %l inode "
			       "%llu\n", (long long)vcn,
	...
	if (sle64_to_cpu(ib->index_block_vcn) != vcn) {
		...
		ntfs_log_error("Corrupt index block: VCN (%l) is different "
			       "from expected VCN (%l) in inode %llu\n",
	...
	if (ib_size != icx->block_size) {
		...
		ntfs_log_error("Corrupt index block : VCN (%l) of inode %llu "
...
descend_into_child_node:
	...
	ntfs_log_debug("Descend into node with VCN %l\n", (long long)vcn);
				  ...
				  INDEX_HEADER_FLAGS node_type)
{
	...
	ntfs_log_trace("ib_vcn: %l ib_size: %u\n", (long long)ib_vcn, ib_size);
	
	ib = (INDEX_BLOCK*) ntfs_calloc(ib_size);
	if (!ib)
		return NULL;
...
static VCN ntfs_ibm_get_free(ntfs_index_context *icx)
{
	...
	bm = (u8 *) ntfs_attr_readall(icx->ni, AT_BITMAP,  icx->name, icx->name_len,
			       &size);
	if (!bm)
		return (VCN)-1;
...
out:	
	ntfs_log_trace("allocated vcn: %l\n", (long long)vcn);
...
static int ntfs_ie_add_vcn(INDEX_ENTRY **ie)
{
	...
	p = (INDEX_ENTRY *) realloc(old, le16_to_cpu(old->length));
	if (!p)
		return STATUS_ERROR;
...
int ntfs_index_add_filename(ntfs_inode *ni, FILE_NAME_ATTR *fn, MFT_REF mref)
{
	...
	ie = (INDEX_ENTRY *) ntfs_calloc(ie_size);
	if (!ie)
		return -1;
