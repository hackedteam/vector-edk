...
{
	if (NInoDirty(ni))
		ntfs_log_error("Releasing dirty inode %l!\n", 
			       (long long)ni->mft_no);
...
static ntfs_inode *ntfs_inode_real_open(ntfs_volume *vol, const MFT_REF mref)
{
	...
	ntfs_log_enter("Entering for inode %l\n", (long long)MREF(mref));
	if (!vol) {
		errno = EINVAL;
		goto out;
	}
	...
	if (l > 0x40000) {
		errno = EIO;
		ntfs_log_perror("Too large attrlist attribute (%l), inode "
				"%l", (long long)l, (long long)MREF(mref));
		goto put_err_out;
	}
	ni->attr_list_size = l;
	...
	if (l != ni->attr_list_size) {
		errno = EIO;
		ntfs_log_perror("Unexpected attrlist size (%l <> %u), inode "
				"%l", (long long)l, ni->attr_list_size, 
				(long long)MREF(mref));
		goto put_err_out;
	}
get_size:
	...
	if (ni->nr_extents > 0) {
		...
		for (i = 0; i < base_ni->nr_extents; ++i) {
			...
			if (base_ni->nr_extents) {
				...
				tmp_nis = (ntfs_inode **) realloc(tmp_nis, base_ni->nr_extents *
						  sizeof(ntfs_inode *));
		 ...
		 */
		if (i != -1)
			ntfs_log_error("Extent inode %l was not found\n",
				       (long long)ni->mft_no);
	}
...
ntfs_inode *ntfs_extent_inode_open(ntfs_inode *base_ni, const MFT_REF mref)
{
	...
	ntfs_log_enter("Opening extent inode %l (base mft record %l).\n",
...
static int ntfs_inode_sync_standard_information(ntfs_inode *ni)
{
	...
	ntfs_log_trace("Entering for inode %l\n", (long long)ni->mft_no);
	...
	if (ntfs_attr_lookup(AT_STANDARD_INFORMATION, AT_UNNAMED,
			     0, CASE_SENSITIVE, 0, NULL, 0, ctx)) {
		ntfs_log_perror("Failed to sync standard info (inode %l)",
				(long long)ni->mft_no);
		ntfs_attr_put_search_ctx(ctx);
		return -1;
	}
	...
	if (!test_nino_flag(ni, NI_TimesSet)) {
...
static int ntfs_inode_sync_file_name(ntfs_inode *ni, ntfs_inode *dir_ni)
{
	...
	while (!ntfs_attr_lookup(AT_FILE_NAME, NULL, 0, 0, 0, NULL, 0, ctx)) {
		...
		if (!index_ni) {
			if (!err)
				err = errno;
			ntfs_log_perror("Failed to open inode %l with index",
		...
		if (!ictx) {
			if (!err)
				err = errno;
			ntfs_log_perror("Failed to get index ctx, inode %l",
		...
		if (ntfs_index_lookup(fn, sizeof(FILE_NAME_ATTR), ictx)) {
			...
			ntfs_log_perror("Index lookup failed, inode %l",
					(long long)index_ni->mft_no);
	...
	if (errno != ENOENT) {
		err = errno;
		ntfs_log_perror("Attribute lookup failed, inode %l",
				(long long)ni->mft_no);
		goto err_out;
	}
	ntfs_attr_put_search_ctx(ctx);
...
static int ntfs_inode_sync_in_dir(ntfs_inode *ni, ntfs_inode *dir_ni)
{
	...
	if ((ni->mrec->flags & MFT_RECORD_IN_USE) && ni->nr_extents != -1 &&
		...
		ntfs_log_perror("Failed to sync FILE_NAME (inode %l)",
				(long long)ni->mft_no);
...
sync_inode:
	...
	if (NInoTestAndClearDirty(ni)) {
		if (ntfs_mft_record_write(ni->vol, ni->mft_no, ni->mrec)) {
			...
			NInoSetDirty(ni);
			ntfs_log_perror("MFT record sync failed, inode %l",
					(long long)ni->mft_no);
		}
	}
...
int ntfs_inode_add_attrlist(ntfs_inode *ni)
{
	...
	if (errno != ENOENT) {
		err = errno;
		ntfs_log_perror("%s: Attribute lookup failed, inode %l",
...
int ntfs_inode_free_space(ntfs_inode *ni, int size)
{
	...
	ntfs_log_trace("Entering for inode %l, size %d\n",
...
int ntfs_inode_get_times(ntfs_inode *ni, char *value, size_t size)
{
	...
	if (ctx) {
		if (ntfs_attr_lookup(AT_STANDARD_INFORMATION, AT_UNNAMED,
				     0, CASE_SENSITIVE, 0, NULL, 0, ctx)) {
			ntfs_log_perror("Failed to get standard info (inode %l)",
					(long long)ni->mft_no);
		} else {
			std_info = (STANDARD_INFORMATION *)((u8 *)ctx->attr +
					le16_to_cpu(ctx->attr->value_offset));
		...
		if (ctx) {
			if (ntfs_attr_lookup(AT_STANDARD_INFORMATION,
				...
				if (cnt)
					...
					ntfs_log_perror("Failed to get file names (inode %l)",
						(long long)ni->mft_no);
				}
			}
			ntfs_attr_put_search_ctx(ctx);
