...
typedef struct _ntfs_md {
    char name[32];                      /* Mount name (can be accessed as "name:/") */
     struct _NTFS_VOLUME *interface;    /* Block device containing the mounted partition */
 ...
 */
extern int ntfsFindPartitions (struct _NTFS_VOLUME *interface, sec_t **partitions);
 ...
 */
extern int ntfsMountDevice (struct _NTFS_VOLUME *interface, ntfs_md **mounts, u32 flags);
...
extern struct _ntfs_vd *ntfsMount (const char *name, struct _NTFS_VOLUME *interface, sec_t startSector, u32 cachePageCount, u32 cachePageSize, u32 flags);
