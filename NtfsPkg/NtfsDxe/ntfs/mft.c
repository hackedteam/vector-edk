		...
		const s64 count, MFT_RECORD *b)
{
	...
	if (!vol || !vol->mft_na || !b || count < 0) {
		errno = EINVAL;
		ntfs_log_perror("%s: b=%p  count=%l  mft=%llu", __FUNCTION__,
	...
	if (br != count) {
		if (br != -1)
			errno = EIO;
		ntfs_log_perror("Failed to read of MFT, mft=%llu count=%l "
				"br=%l", (long long)m, (long long)count,
				(long long)br);
		return -1;
	}
	return 0;
...
int ntfs_mft_record_format(const ntfs_volume *vol, const MFT_REF mref)
{
	...
	m = (MFT_RECORD *) ntfs_calloc(vol->mft_record_size);
	if (!m)
		goto out;
...
static int ntfs_mft_bitmap_find_free_rec(ntfs_volume *vol, ntfs_inode *base_ni)
{
	...
	ntfs_log_debug("Starting bitmap search: pass %u, pass_start 0x%lx, "
			"pass_end 0x%lx, data_pos 0x%lx.\n", pass,
	...
	for (; pass <= 2; size = PAGE_SIZE) {
		...
		ntfs_log_debug("Read 0x%lx bytes.\n", (long long)ll);
		...
		if (pass == 2) {
			...
			ntfs_log_debug("pass %i, pass_start 0x%lx, pass_end "
...
put_err_out:
	...
	ntfs_inode_mark_dirty(ctx->ntfs_ino);
	ntfs_attr_put_search_ctx(ctx);
	ntfs_log_debug("Restored status of mftbmp: allocated_size 0x%lx, "
			"data_size 0x%lx, initialized_size 0x%lx.\n",
...
static int ntfs_mft_data_extend_allocation(ntfs_volume *vol)
{
	...
	lcn = rl->lcn + rl->length;
	ntfs_log_debug("Last lcn of mft data attribute is 0x%lx.\n", (long long)lcn);
	...
	do {
		...
		if (errno != ENOSPC || nr == min_nr) {
			ntfs_log_perror("Failed to allocate (%l) clusters "
	...
	ntfs_log_debug("Allocated %l clusters.\n", (long long)nr);
...
static int ntfs_mft_record_init(ntfs_volume *vol, s64 size)
{
	...
	ntfs_log_debug("Status of mft data before extension: "
			"allocated_size 0x%lx, data_size 0x%lx, "
			"initialized_size 0x%lx.\n",
	...
	while (size > mft_na->initialized_size) {
		...
		ntfs_log_debug("Initializing mft record 0x%lx.\n", (long long)ll2);
...
static int ntfs_mft_rec_init(ntfs_volume *vol, s64 size)
{
	...
	if (size > mft_na->allocated_size || size > mft_na->initialized_size) {
		...
		ntfs_log_error("$MFT: size=%l  allocated_size=%l  "
			       "data_size=%l  initialized_size=%l\n",
			       (long long)size,
			       (long long)mft_na->allocated_size,
			       (long long)mft_na->data_size,
			       (long long)mft_na->initialized_size);
...
found_free_rec:
	...
	if (ntfs_is_file_record(m->magic) && (m->flags & MFT_RECORD_IN_USE)) {
		ntfs_log_error("(2) Inode %l is used but it wasn't marked in "
			       "$MFT bitmap. Fixed.\n", (long long)bit);
		free(m);
		goto undo_mftbmp_alloc;
	}
	...
	ntfs_log_error("allocated %sinode %l\n",
...
retry:	
	...
	bit = ntfs_mft_bitmap_find_free_rec(vol, base_ni);
	if (bit >= 0) {
		ntfs_log_debug("found free record (#1) at %l\n",
				(long long)bit);
		goto found_free_rec;
	}
	if (errno != ENOSPC)
	...
	if (mftbmp_na->initialized_size << 3 > ll &&
		...
		ntfs_log_debug("found free record (#2) at %l\n",
				(long long)bit);
		goto found_free_rec;
	}
	/*
	 ...
	 */
	ntfs_log_debug("Status of mftbmp before extension: allocated_size 0x%lx, "
			"data_size 0x%lx, initialized_size 0x%lx.\n",
			(long long)mftbmp_na->allocated_size,
			(long long)mftbmp_na->data_size,
			(long long)mftbmp_na->initialized_size);
	...
	ntfs_log_debug("Status of mftbmp after initialized extension: "
			...
			(long long)mftbmp_na->data_size,
			(long long)mftbmp_na->initialized_size);
	ntfs_log_debug("found free record (#3) at %l\n", (long long)bit);
found_free_rec:
	...
	/* Sanity check that the mft record is really not in use. */
	if (ntfs_is_file_record(m->magic) && (m->flags & MFT_RECORD_IN_USE)) {
		ntfs_log_error("(1) Inode %l is used but it wasn't marked in "
			       "$MFT bitmap. Fixed.\n", (long long)bit);
		free(m);
		goto retry;
	}
	...
	if (!base_ni)
		vol->mft_data_pos = bit + 1;
	/* Return the opened, allocated inode of the allocated mft record. */
	ntfs_log_debug("allocated %sinode 0x%lx.\n",
			base_ni ? "extent " : "", (long long)bit);
...
int ntfs_mft_record_free(ntfs_volume *vol, ntfs_inode *ni)
{
	...
	ntfs_log_trace("Entering for inode 0x%lx.\n", (long long) ni->mft_no);
