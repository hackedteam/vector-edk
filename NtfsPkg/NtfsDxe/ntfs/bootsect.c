...
int ntfs_boot_sector_parse(ntfs_volume *vol, const NTFS_BOOT_SECTOR *bs)
{
	...
	ntfs_log_debug("NumberOfSectors = %l\n", (long long)sectors);
	if (!sectors) {
	...
	if (vol->dev->d_ops->seek(vol->dev, 
				  ...
				  SEEK_SET) == -1) {
		ntfs_log_perror("Failed to read last sector (%l)",
			       	(long long)sectors);
	...
	ntfs_log_debug("MFT LCN = %l\n", (long long)vol->mft_lcn);
	ntfs_log_debug("MFTMirr LCN = %l\n", (long long)vol->mftmirr_lcn);
	if ((vol->mft_lcn     < 0 || vol->mft_lcn     > vol->nr_clusters) ||
	    (vol->mftmirr_lcn < 0 || vol->mftmirr_lcn > vol->nr_clusters)) {
		ntfs_log_error("$MFT LCN (%l) or $MFTMirr LCN (%l) is "
			      "greater than the number of clusters (%l).\n",
