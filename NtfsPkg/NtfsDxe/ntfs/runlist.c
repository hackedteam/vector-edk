					...
					int new_size)
{
	...
	if (old_size == new_size)
		return rl;
	return (runlist_element *) realloc(rl, new_size);
}
					      ...
					      runlist_element *srl)
{
	...
	if (marker) {
		...
		if (drl[ds].vcn <= marker_vcn) {
			...
			if (drl[ds].vcn == marker_vcn) {
				ntfs_log_debug("Old marker = %l, replacing with "
						"LCN_ENOENT.\n",
		...
		const ATTR_RECORD *attr, runlist_element *old_rl)
{
	...
	while (buf < attr_end && *buf) {
		...
		if ((int)((rlpos + 3) * sizeof(*old_rl)) > rlsize) {
			...
			rlsize += 0x1000;
			rl2 = (runlist_element *) realloc(rl, rlsize);
			if (!rl2) {
				int eo = errno;
				free(rl);
				errno = eo;
		...
		const runlist_element *rl, const VCN start_vcn, int max_size)
{
	...
	if (start_vcn < 0) {
		ntfs_log_trace("start_vcn %l (should be >= 0)\n",
				(long long) start_vcn);
		errno = EINVAL;
		goto errno_set;
	}
	if (!rl) {
		if (start_vcn) {
			ntfs_log_trace("rl NULL, start_vcn %l (should be > 0)\n",
					(long long) start_vcn);
			errno = EINVAL;
			goto errno_set;
		}
