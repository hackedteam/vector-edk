...
#define MFT_REC_SPACE_FILLER		0xffff /* Just to make flags
						     16-bit. */
...
#define ATTR_FLAGS	unsigned short
 ...
 */
		...
		/* 72*/ void *v1_end[0]; /* Marker for offsetof(). */
		} /**/;
	};
...
#define SECURITY_DESCRIPTOR_CONTROL short
...
#define 	SMALL_INDEX	0 /* The index is small enough to fit inside the
				index root attribute and there is no index
				allocation attribute present. */
#define	LARGE_INDEX	1  /* The index is too large to fit in the index
	 ...
	 */
#define	LEAF_NODE	0 /* This is a leaf node, i.e. there are no more
				nodes branching off it. */
#define	INDEX_NODE	1 /* This node indexes other nodes, i.e. is not a
				leaf node. */
 ...
 */
#define INDEX_ENTRY_FLAGS short
#define INDEX_ENTRY_NODE const_cpu_to_le16(1) /* This entry contains a
					...
					number (see below). */
#define	INDEX_ENTRY_END  const_cpu_to_le16(2) /* This signifies the last
					entry in an index block. The index
					entry does not represent a file but it
					can point to a sub-node. */
#define	INDEX_ENTRY_SPACE_FILLER = 0xffff /* Just to force 16-bit width. */
