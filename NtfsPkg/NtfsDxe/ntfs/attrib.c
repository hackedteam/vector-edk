		...
		const ATTR_RECORD *a, u8 *b)
{
	...
	for (i = 0, total = 0; rl[i].length; i++) {
		if (total + (rl[i].length << vol->cluster_size_bits) >=
			 ...
			 */
			intbuf = (unsigned char*) ntfs_malloc(rl[i].length << vol->cluster_size_bits);
			if (!intbuf) {
				free(rl);
				return 0;
			}
		...
		ntfschar *name, u32 name_len)
{
	...
	ntfs_log_enter("Entering for inode %l, attr 0x%x.\n",
	...
	na = (ntfs_attr *) ntfs_calloc(sizeof(ntfs_attr));
	if (!na)
		goto out;
	...
	if (na->type == AT_DATA && na->name == AT_UNNAMED &&
		...
		errno = EIO;
		ntfs_log_perror("Inode %l has corrupt attribute flags "
	...
	if (a->non_resident) {
		if ((a->flags & ATTR_COMPRESSION_MASK)
				 && !a->compression_unit) {
			errno = EIO;
			ntfs_log_perror("Compressed inode %l attr 0x%x has "
					"no compression unit",
					(unsigned long long)ni->mft_no, type);
			goto put_err_out;
		}
...
int ntfs_attr_map_runlist(ntfs_attr *na, VCN vcn)
{
	...
	ntfs_log_trace("Entering for inode 0x%lx, attr 0x%x, vcn 0x%lx.\n",
...
int ntfs_attr_map_whole_runlist(ntfs_attr *na)
{
	...
	if (highest_vcn && highest_vcn != last_vcn - 1) {
		...
		ntfs_log_perror("Failed to load full runlist: inode: %llu "
				"highest_vcn: 0x%lx last_vcn: 0x%lx",
				(unsigned long long)na->ni->mft_no, 
...
LCN ntfs_attr_vcn_to_lcn(ntfs_attr *na, const VCN vcn)
{
	...
	ntfs_log_trace("Entering for inode 0x%lx, attr 0x%x.\n", (unsigned long
...
runlist_element *ntfs_attr_find_vcn(ntfs_attr *na, const VCN vcn)
{
	...
	ntfs_log_trace("Entering for inode 0x%lx, attr 0x%x, vcn %lx\n",
		       (unsigned long long)na->ni->mft_no, na->type,
		       (long long)vcn);
retry:
	rl = na->rl;
...
res_err_out:
	...
	for (; count; rl++, ofs = 0) {
		...
		to_read = min(count, (rl->length << vol->cluster_size_bits) -
				ofs);
retry:
		ntfs_log_trace("Reading %l bytes from vcn %l, lcn %l, ofs"
...
s64 ntfs_attr_pread(ntfs_attr *na, const s64 pos, s64 count, void *b)
{
	...
	if (!na || !na->ni || !na->ni->vol || !b || pos < 0 || count < 0) {
		errno = EINVAL;
		ntfs_log_perror("%s: na=%p  b=%p  pos=%l  count=%l",
				__FUNCTION__, na, b, (long long)pos,
				(long long)count);
		return -1;
	}
	
	ntfs_log_enter("Entering for inode %l attr 0x%x pos %l count "
		       "%l\n", (unsigned long long)na->ni->mft_no,
...
static int ntfs_attr_fill_zero(ntfs_attr *na, s64 pos, s64 count)
{
	...
	buf = (char *) ntfs_calloc(NTFS_BUF_SIZE);
	if (!buf)
		goto err_out;
			       ...
			       runlist_element **rl, VCN *update_from)
{
	...
	ntfs_log_trace("count: %l, cur_vcn: %l, from: %l, to: %l, ofs: "
	...
	*rl = ntfs_attr_find_vcn(na, cur_vcn);
	...
	if ((na->data_flags & ATTR_COMPRESSION_MASK)
		...
		need = (alloc_vcn | (na->compression_block_clusters - 1))
			+ 1 - alloc_vcn;
		if (need > (*rl)->length) {
			ntfs_log_error("Cannot allocate %l clusters"
					" within a hole of %l\n",
					(long long)need,
					(long long)(*rl)->length);
			errno = EIO;
...
s64 ntfs_attr_pwrite(ntfs_attr *na, const s64 pos, s64 count, const void *b)
{
	...
	ntfs_log_enter("Entering for inode %l, attr 0x%x, pos 0x%lx, count "
	...
	if (pos + count > na->initialized_size) {
		...
		if (na->ni->mrec->flags & MFT_RECORD_IS_DIRECTORY
			...
			set_nino_flag(na->ni,NI_KnownSize);
		}
#endif
		ntfs_attr_put_search_ctx(ctx);
		ctx = NULL;
	...
	for (hole_end = 0; count; rl++, ofs = 0) {
		...
		if (rl->lcn < (LCN)0) {
			...
			if (rl->lcn != (LCN)LCN_HOLE) {
				errno = EIO;
				ntfs_log_perror("%s: Unexpected LCN (%l)", 
						__FUNCTION__,
						(long long)rl->lcn);
				goto rl_err_out;
			}
		...
		to_write = min(count, (rl->length << vol->cluster_size_bits) - ofs);
retry:
		ntfs_log_trace("Writing %l bytes to vcn %l, lcn %l, ofs "
...
int ntfs_attr_pclose(ntfs_attr *na)
{
	...
	ntfs_log_enter("Entering for inode 0x%lx, attr 0x%x.\n",
		...
		const u32 bk_size, void *dst)
{
	...
	ntfs_log_trace("Entering for inode 0x%lx, attr type 0x%x, pos 0x%lx.\n",
			(unsigned long long)na->ni->mft_no, na->type,
			(long long)pos);
		...
		const u32 bk_size, void *src)
{
	...
	if (written <= 0) {
		ntfs_log_perror("%s: written=%l", __FUNCTION__,
				(long long)written);
	}
		...
		const u8 *val, const u32 val_len, ntfs_attr_search_ctx *ctx)
{
	...
	errno = EIO;
	ntfs_log_perror("%s: Corrupt inode (%l)", __FUNCTION__, 
		...
		ntfs_attr_search_ctx *ctx)
{
	...
	ntfs_log_trace("Entering for inode %l, attribute type 0x%x.\n",
			(unsigned long long)ni->mft_no, type);
	if (!base_ni) {
...
do_next_attr:
	...
	errno = EIO;
	ntfs_log_perror("Inode is corrupt (%l)", (long long)base_ni->mft_no);
	return -1;
not_found:
	/*
		...
		const s64 size)
{
	...
	if (size < 0) {
		errno = EINVAL;
		ntfs_log_perror("%s: size=%l", __FUNCTION__,
				(long long)size);
		return -1;
	}
	...
	if (type == AT_ATTRIBUTE_LIST && size > 0x40000) {
		errno = ERANGE;
		ntfs_log_perror("Too large attrlist (%l)", (long long)size);
		return -1;
	}
			...
			u32 size, ATTR_FLAGS data_flags)
{
	...
	ntfs_log_trace("Entering for inode 0x%lx, attr 0x%x, flags 0x%x.\n",
		...
		ATTR_FLAGS flags)
{
	...
	ntfs_log_trace("Entering for inode 0x%lx, attr 0x%x, lowest_vcn %l, "
		...
		ntfschar *name, u8 name_len, const u8 *val, s64 size)
{
	...
	if (!ni || size < 0 || type == AT_ATTRIBUTE_LIST) {
		errno = EINVAL;
		ntfs_log_perror("%s: ni=%p  size=%l", __FUNCTION__, ni,
				(long long)size);
		return -1;
	}

	ntfs_log_trace("Entering for inode %l, attr %x, size %l.\n",
			...
			BOOL force_non_resident)
{
	...
	ntfs_log_trace("Inode 0x%lx attr 0x%x new size %l\n", 
		       (unsigned long long)na->ni->mft_no, na->type, 
		       (long long)newsize);
					...
					hole_type holes)
{
	...
	ntfs_log_trace("Inode %l, attr 0x%x, new size %l old size %l\n",
			(unsigned long long)na->ni->mft_no, na->type,
					...
					hole_type holes)
{
	...
	ntfs_log_enter("Entering for inode %l, attr 0x%x, size %l\n",
		       (unsigned long long)na->ni->mft_no, na->type, 
		       (long long)newsize);
		...
		int stream_name_len, const char *buf, size_t size, off_t offset)
{
	...
	while (size) {
		...
		if (res < (s64)size)
			ntfs_log_perror("ntfs_attr_pwrite partial write (%l: "
				"%l <> %d)", (long long)offset,
				(long long)size, res);
		if (res <= 0) {
			res = -errno;
			goto exit;
	...
	if (!na) {
		...
		if (type != AT_DATA) {
			ntfs_log_perror("Failed to open attribute 0x%02x of inode "
				"0x%lx", type, (unsigned long long)ni->mft_no);
		}
		return -1;
	}
