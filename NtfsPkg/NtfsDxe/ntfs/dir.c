			...
			const struct CACHED_GENERIC *wanted)
{
	return (!cached->variable
		    || strcmp((const char *) cached->variable, (const char *) wanted->variable));
}
		...
		const ntfschar *uname, const int uname_len)
{
	...
	if (ntfs_attr_lookup(AT_INDEX_ROOT, NTFS_INDEX_I30, 4, CASE_SENSITIVE, 0, NULL,
		...
		ntfs_log_perror("Index root attribute missing in directory inode "
				"%l", (unsigned long long)dir_ni->mft_no);
		goto put_err_out;
	}
	...
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		...
		if ((u8*)ie < (u8*)ctx->mrec || (u8*)ie +
				...
				index_end) {
			ntfs_log_error("Index entry out of bounds in inode %l"
		...
		if (!le16_to_cpu(ie->length)) {
			ntfs_log_error("Zero length index entry in inode %l"
				       "\n", (unsigned long long)dir_ni->mft_no);
			goto put_err_out;
		}
		/*
	...
	if (!ia_na) {
		ntfs_log_perror("Failed to open index allocation (inode %l)",
				(unsigned long long)dir_ni->mft_no);
		goto put_err_out;
	}
...
descend_into_child_node:
	...
	if (br != 1) {
		if (br != -1)
			errno = EIO;
		ntfs_log_perror("Failed to read vcn 0x%lx",
			       	(unsigned long long)vcn);
		goto close_err_out;
	}

	if (sle64_to_cpu(ia->index_block_vcn) != vcn) {
		ntfs_log_error("Actual VCN (0x%lx) of index buffer is different "
				"from expected VCN (0x%lx).\n",
	...
	if (le32_to_cpu(ia->index.allocated_size) + 0x18 != index_block_size) {
		ntfs_log_error("Index buffer (VCN 0x%lx) of directory inode 0x%lx "
	...
	if (index_end > (u8*)ia + index_block_size) {
		ntfs_log_error("Size of index buffer (VCN 0x%lx) of directory inode "
...
out:
	...
	ntfs_log_debug("ntfs_pathname_to_inode leaving... %x (MFT no %x)\n\r", result, (ni != NULL) ? (UINTN) ni->mft_no : 0);
	ntfs_log_debug("free(ascii)\n\r");
	free(ascii);
	ntfs_log_debug("free(unicode)\n\r");
	free(unicode);
	if (err)
		errno = err;
	ntfs_log_debug("return\n\r");
	return result;
}
...
#define INDEX_TYPE_ROOT 0
#define INDEX_TYPE_ALLOCATION 1
					...
					FILE_ATTR_FLAGS attributes)
{
	...
	if (ni) {
		...
		if (ntfs_inode_close(ni)) {
			...
			ntfs_log_error("Failed to close inode %l\n",
				(long long)MREF(mref));
		}
	}
	if (dt_type == NTFS_DT_UNKNOWN)
		ntfs_log_error("Could not decode the type of inode %l\n",
				(long long)MREF(mref));
	return (dt_type);
}
...
static MFT_REF ntfs_mft_get_parent_ref(ntfs_inode *ni)
{
	...
	if (ntfs_attr_lookup(AT_FILE_NAME, AT_UNNAMED, 0, 0, 0, NULL, 0, ctx)) {
		ntfs_log_error("No file name found in inode %l\n", 
	...
	if ((u8*)fn +	le32_to_cpu(ctx->attr->value_length) >
		...
		ntfs_log_error("Corrupt file name attribute in inode %l.\n",
			       (unsigned long long)ni->mft_no);
		goto io_err_out;
	}
		...
		void *dirent, ntfs_filldir_t filldir)
{
	...
	ntfs_log_trace("Entering for inode %l, *pos 0x%lx.\n",
	...
	if (!ia_na) {
		if (errno != ENOENT) {
			ntfs_log_perror("Failed to open index allocation attribute. "
				"Directory inode %l is corrupt or bug",
				(unsigned long long)dir_ni->mft_no);
			return -1;
		}
		i_size = 0;
...
find_next_index_buffer:
	...
	ntfs_log_debug("Handling index block 0x%lx.\n", (long long)bmp_pos);
	...
	if (sle64_to_cpu(ia->index_block_vcn) != ia_start >>
			index_vcn_size_bits) {
		ntfs_log_error("Actual VCN (0x%lx) of index buffer is different "
				"from expected VCN (0x%lx) in inode 0x%lx.\n",
				(long long)sle64_to_cpu(ia->index_block_vcn),
	...
	if (le32_to_cpu(ia->index.allocated_size) + 0x18 != index_block_size) {
		ntfs_log_error("Index buffer (VCN 0x%lx) of directory inode %l "
				"has a size (%u) differing from the directory "
				"specified size (%u).\n", (long long)ia_start >>
				index_vcn_size_bits,
				(unsigned long long)dir_ni->mft_no,
	 ...
	 */
	for (;; ie = (INDEX_ENTRY*)((u8*)ie + le16_to_cpu(ie->length))) {
		ntfs_log_debug("In index allocation, offset 0x%lx.\n",
...
EOD:
	...
	*pos = i_size + vol->mft_record_size;
	...
	if (ia_na)
		ntfs_attr_close(ia_na);
	ntfs_log_debug("EOD, *pos 0x%lx, returning 0.\n", (long long)*pos);
	return 0;
dir_err_out:
	errno = EIO;
err_out:
		...
		const ntfschar *target, int target_len)
{
	...
	si = (STANDARD_INFORMATION *) ntfs_calloc(si_len);
	if (!si) {
		err = errno;
		goto err_out;
	}
	...
	if (S_ISDIR(type)) {
		...
		ir = (INDEX_ROOT *) ntfs_calloc(ir_len);
		if (!ir) {
			err = errno;
			goto err_out;
		}
		...
		switch (type) {
			...
			case S_IFCHR:
				...
				data->major = 0; //cpu_to_le64(major(dev));
				data->minor = 0; // cpu_to_le64(minor(dev));
				if (type == S_IFBLK)
	...
	fn = (FILE_NAME_ATTR *) ntfs_calloc(fn_len);
	if (!fn) {
		err = errno;
		goto err_out;
	}
