			...
			le16 inherited)
{
	...
	for (nace = 0; nace < oldcnt; nace++) {
		...
		if ((poldace->flags & selection)
			 ...
			 */
					pnewace->mask |= (ACCESS_MASK) (OWNER_RIGHTS
							| FILE_READ
							| FILE_WRITE
							| FILE_EXEC
							| cpu_to_le32(0x40));
	       ...
	       const SID * usid, const SID * gsid)
{
	...
	grants = OWNER_RIGHTS;
	if (isdir) {
		gflags = (ACE_FLAGS )DIR_INHERITANCE;
		if (mode & S_IXUSR)
			grants |= DIR_EXEC;
		if (mode & S_IWUSR)
			grants |= DIR_WRITE;
	...
	if (!adminowns) {
		...
		if (denials) {
			...
			pdace->mask = (ACCESS_MASK) denials;
