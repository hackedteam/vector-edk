			    ...
			    u8 *writeback)
{
	...
	*writeback = 0;
	...
	if (written != size) {
		if (!written)
			errno = EIO;
		ntfs_log_perror("Bitmap write error (%l, %l)",
		...
		LCN start_lcn, const NTFS_CLUSTER_ALLOCATION_ZONES zone)
{
	...
	ntfs_log_enter("Entering with count = 0x%lx, start_lcn = 0x%lx, "
	...
	if (!vol || count < 0 || start_lcn < -1 || !vol->lcnbmp_na ||
		...
		errno = EINVAL;
		ntfs_log_perror("%s: vcn: %l, count: %l, lcn: %l", 
...
int ntfs_cluster_free_from_rl(ntfs_volume *vol, runlist *rl)
{
	...
	for (; rl->length; rl++) {
		...
		ntfs_log_trace("Dealloc lcn 0x%lx, len 0x%lx.\n",
...
out:
	...
	if (vol->free_clusters > vol->nr_clusters)
		ntfs_log_error("Too many free clusters (%l > %l)!",
			       (long long)vol->free_clusters, 
...
int ntfs_cluster_free(ntfs_volume *vol, ntfs_attr *na, VCN start_vcn, s64 count)
{
	...
	ntfs_log_enter("Entering for inode 0x%lx, attr 0x%x, count 0x%lx, "
	...
	if (rl->lcn < 0 && rl->lcn != LCN_HOLE) {
		errno = EIO;
		ntfs_log_perror("%s: Unexpected lcn (%l)", __FUNCTION__, 
				(long long)rl->lcn);
		goto leave;
	}
	...
	for (; rl->length && count != 0; ++rl) {
		...
		if (rl->lcn < 0 && rl->lcn != LCN_HOLE) {
			...
			errno = EIO;
			ntfs_log_perror("%s: Invalid lcn (%l)", 
	...
	if (count != -1 && count != 0) {
		// FIXME: Eeek! BUG()
		errno = EIO;
		ntfs_log_perror("%s: count still not zero (%l)", __FUNCTION__,
			       (long long)count);
		goto out;
	}
