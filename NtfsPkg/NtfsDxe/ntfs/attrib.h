 ...
 */
struct _ntfs_attr {
	runlist_element *rl;		/* 0x00 */
	ntfs_inode *ni;				/* 0x04 */
	ATTR_TYPES type;			/* 0x08 */
	ATTR_FLAGS data_flags;		/* 0x0c */
	ntfschar *name;				/* 0x10 */
	u32 name_len;				/* 0x14 */
	unsigned long state;		/* 0x18 */
	s64 allocated_size;			/* 0x1c */
	s64 data_size;				/* 0x24 */
	s64 initialized_size;		/* 0x2c */
	s64 compressed_size;		/* 0x34 */
	u32 compression_block_size;	/* 0x3c */
	u8 compression_block_size_bits; /* 0x40 */
	u8 compression_block_clusters;  /* 0x41 */
