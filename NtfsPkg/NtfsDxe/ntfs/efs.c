	...
	if (ni) {
		if (ni->flags & FILE_ATTR_ENCRYPTED) {
			...
			if (efs_info
				...
				if (efs_info) {
					free(efs_info);
					ntfs_log_error("Bad efs_info for inode %l\n",
						(long long)ni->mft_no);
				} else {
			...
			errno = ENODATA;
			ntfs_log_trace("Inode %l is not encrypted\n",
				(long long)ni->mft_no); 
		}
	}
			...
			
{
	...
	if (ni && value && size) {
		if (ni->flags & (FILE_ATTR_ENCRYPTED | FILE_ATTR_COMPRESSED)) {
			if (ni->flags & FILE_ATTR_ENCRYPTED) {
				ntfs_log_trace("Inode %l already encrypted\n",
						(long long)ni->mft_no);
				errno = EEXIST;
			} else {
				/*
				 ...
				 */
				ntfs_log_error("Inode %l cannot be encrypted and compressed\n",
					(long long)ni->mft_no);
				errno = EIO;
			}
			return -1;
...
int ntfs_efs_fixup_attribute(ntfs_attr_search_ctx *ctx, ntfs_attr *na) 
{
	...
	if (oldsize) {
		...
		if (padding_length > 511 || padding_length > na->data_size-2) {
			errno = EINVAL;
			ntfs_log_error("invalid padding length %d for data_size %l\n",
