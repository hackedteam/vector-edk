				       ...
				       s64 count, int value)
{
	...
	if (!na || start_bit < 0 || count < 0) {
		errno = EINVAL;
		ntfs_log_perror("%s: Invalid argument (%p, %l, %l)",
	...
	if (!buf)
	{
		ntfs_log_perror("ntfs_malloc failed");
		return -1;
	}
	...
	do {
		...
		if (br != bufsize) {
			...
			ntfs_log_perror("Failed to write buffer to bitmap "
				"(%l != %l). Leaving inconsistent metadata",
		...
		if (lastbyte && count != 0) {
			...
			ntfs_log_error("Last buffer but count is not zero "
				       "(%l). Leaving inconsistent metadata.\n",
				       (long long)count);
			errno = EIO;
			goto free_err_out;
		}
...
int ntfs_bitmap_set_run(ntfs_attr *na, s64 start_bit, s64 count)
{
	...
	ntfs_log_enter("Set from bit %l, count %l\n",
...
int ntfs_bitmap_clear_run(ntfs_attr *na, s64 start_bit, s64 count)
{
	...
	ntfs_log_enter("Clear from bit %l, count %l\n",
		       (long long)start_bit, (long long)count);
